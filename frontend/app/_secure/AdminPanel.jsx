...
import { adminApi, adminLogout, adminUploadApi, api, applyTheme, clearToken, downloadInvoice, inr, itemSelectionText, itemVariantText, openInvoice, productImage, refreshAdminSession, setThemeMode, setToken, storedThemeMode, token } from '../lib';
...
const navSections = [
  ...
  { label: 'Delivery', items: ['delivery-boys', 'delivery-slabs'] },
...
const featureControlKeys = ['customer_theme_enabled', 'customer_dark_mode_enabled', 'online_ordering_enabled', 'delivery_enabled', 'takeaway_enabled', 'razorpay_enabled', 'google_maps_enabled', 'guest_checkout_enabled', 'customer_login_enabled'];
const hiddenGeneralSettings = new Set(['customer_login_required', 'accept_orders', 'force_close_orders', 'order_schedule', 'customer_default_theme', 'admin_theme_mode', ...featureControlKeys]);
...
export default function AdminPage() {
  ...
  async function toggleAcceptOrders(nextValue) {
    if (nextValue === '0' && !window.confirm('Turn ordering OFF now? Customers will not be able to place new orders.')) return;
    try {
      const data = await adminRequest('/admin/settings/order-availability/master', { method: 'PUT', body: JSON.stringify({ accept_orders: nextValue }) });
      setStoreAvailability(data.availability);
      setMessage(nextValue === '1' ? 'Orders are open.' : 'Orders are closed.');
    } catch (err) {
      setMessage(err.message);
    }
  }

  async function toggleForceClose(checked) {
    if (checked && !window.confirm('Force close orders now? This overrides the daily schedule until you turn it off.')) return;
    try {
      const data = await adminRequest('/admin/settings', { method: 'PUT', body: JSON.stringify({ force_close_orders: checked ? '1' : '0' }) });
      setSettings(current => ({ ...current, ...data.settings }));
      const statusData = await api('/store/status');
      setStoreAvailability(statusData.store || null);
      setMessage(checked ? 'Orders are force closed.' : 'Orders returned to normal schedule.');
    } catch (err) {
      setMessage(err.message);
    }
  }
      ...
      <section className="admin-main">
        <header className="admin-topbar">
          ...
          <div className="admin-topbar-actions">
            <ToggleSwitch
              checked={storeAvailability?.accept_orders !== '0'}
              onChange={checked => toggleAcceptOrders(checked ? '1' : '0')}
              label={storeAvailability?.is_open ? 'Accepting Orders' : 'Orders Closed'}
              description={storeAvailability?.reason === 'FORCE_CLOSED' ? 'Force closed' : storeAvailability?.message}
            />
        ...
        {active === 'order-availability' && (
          <section className="panel">
            <div className="panel-heading"><h2>Order Availability</h2><p>Backend decisions use Asia/Kolkata server time and apply immediately.</p></div>
            <div className="availability-summary-grid">
              <article className="availability-summary-card">
                ...
                <StatusBadge tone={storeAvailability?.is_open ? 'success' : 'danger'}>{storeAvailability?.reason || 'Loading'}</StatusBadge>
              </article>
              <article className="availability-summary-card">
              ...
              <article className="availability-summary-card">
                ...
                <strong>{storeAvailability?.next_opening ? `${statusLabel(storeAvailability.next_opening.day)} ${storeAvailability.next_opening.open}` : 'No upcoming window'}</strong>
                <small>{storeAvailability?.timezone || 'Asia/Kolkata'}</small>
              </article>
            </div>
            <div className="availability-controls">
              <ToggleSwitch
                ...
                onChange={checked => toggleAcceptOrders(checked ? '1' : '0')}
                label="Accept Orders"
                description={storeAvailability?.is_open ? 'Normal ordering is currently available.' : storeAvailability?.message || 'Loading status...'}
              />
              <ToggleSwitch
                checked={(settings.force_close_orders ?? storeAvailability?.force_close_orders ?? '0') === '1'}
                onChange={toggleForceClose}
                label="Force Close Orders"
                description={(settings.force_close_orders ?? storeAvailability?.force_close_orders ?? '0') === '1' ? 'Manual override is blocking new orders.' : 'Normal Schedule'}
                danger
              />
            </div>
